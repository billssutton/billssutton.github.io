d3Visuals.directive('columnchart', function($filter) {
        ...
        link: function(scope, ele, attrs) {
            ...
            scope.$watch('data', function() {
                if (scope.data == undefined || scope.data == null || scope.data ==  [] || scope.data == '') {
                    ...
                    names.length = 0;
                    dataSeriesObject = scope.data.specificData.dataSeries;
                    dataSeriesKeys = dataSeriesObject[0].map(function(d) { return d.x});
                    xAxisText = scope.data.specificData.names[0];
                    names = scope.data.specificData.names;
            ...
            function buildGrouped(transition) {
                if(transition) {
                    rect
                        ...
                        .attr("height", function(d) { return container.height - yScale(d.y); });
                }
                else {
                    rect
                        .attr("x", function(d, i, j) {
            ...
            function resize() {
                ...
                yGroupMax = d3.max(layers, function(lay) { return d3.max(lay, function(d) { return d.y; }); });
                yStackMax = d3.max(layers, function(lay) { return d3.max(lay, function(d) { return d.y0 + d.y; }); });
